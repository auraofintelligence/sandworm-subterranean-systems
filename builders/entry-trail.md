# Entry Trail Builder

Purpose: Choose a doorway into Sandworm without needing to accept the whole vision.

Status: Draft for human review

## Questions

### Which doorway are you using?

Prompt: Making, mapping, filming, documentary research, sources, material loops, or another doorway.

_Not answered yet._

### What question are you carrying?

Prompt: Write it as a real question, not a conclusion.

_Not answered yet._

### What would be useful to leave with?

Prompt: A note, map, source list, experiment card, film prompt, workshop plan, or handoff.

_Not answered yet._

## Boundaries

The trail is an invitation, not a test. People can stay, leave, disagree, or take only the part that helps.

## Next small step

Name one small practical action that would make the idea easier to understand.
